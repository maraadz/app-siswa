...
export default function NegarawanPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const res = await getRekapDimensi(activeThn, activeSmt, 3); // id_dimensi: 3
      ...
      if (res && res.status === 'ok') {
        ...
        const mapped = masterSKL.map((m: any) => {
          ...
          return {
            elemen: m.PELAJARAN || m.SUB_ELEMEN || 'Nasionalisme',
        ...
        // SINKRONISASI: Update storage agar Radar Home tahu data sudah dilihat
        const storageKey = `last_cnt_negarawan_${user?.id}`;
        localStorage.setItem(storageKey, finalData.length.toString());

        setRekapData(finalData);
      }
    } catch (err: any) {
      if (isMounted.current) setError("Gagal sinkronisasi data Negarawan.");
    } finally {
    ...
    <div className="min-h-screen bg-[#F1F5F9]">
      <header className="text-white pt-6 pb-12 px-4 shadow-md" style={{ backgroundColor: warnaSatuan }}>
        <div className="max-w-6xl mx-auto flex items-center gap-4">
          <BackButton to="/" />
          <h1 className="text-xl font-bold">Dimensi Negarawan</h1>
        </div>
      </header>
      ...
      <main className="max-w-6xl mx-auto px-4 -mt-8 pb-10">
        <section className="bg-white rounded-2xl shadow-sm p-6 mb-6 border border-gray-100">
          <div className="flex flex-col sm:flex-row gap-4 items-end">
            <div className="w-full sm:flex-1">
              <label className="text-[10px] font-bold text-gray-400 uppercase ml-1">Periode Semester</label>
        ...
        <div className="bg-white rounded-2xl shadow-sm overflow-hidden border border-gray-100">
          ...
          {loading ? (
            ...
            <DataTable columns={columns} data={rekapData} emptyMessage="Belum ada rekapan nilai Negarawan." />
          )}
        </div>
      </main>
    </div>
